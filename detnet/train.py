...
def train(datasets, args):
    ...
    torchvision_like = args.arch.split(':')[0] in ('torchvision', 'detectron2', 'mmdet')
    ...
    if args.arch == 'pointdet':
        convert_train_target = creat_point_heat_map(model, image_size)
    elif torchvision_like:
        convert_train_target = TorchVisionTrans()
    else:
        convert_train_target = create_match_prior_box(model, image_size, args.bg_iou_threshold, args.fg_iou_threshold,
    ...
    if args.arch in ('pointdet',) or torchvision_like:
