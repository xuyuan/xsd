...
class COCODetection(AnchorBoxDataset):
    ...
    def include(self, image_ids):
        self.ids = [i for i in self.ids if i in image_ids]
        self._update_coco_by_ids()

    def _update_coco_by_ids(self):
        self.coco.anns = {k: v for k, v in self.coco.anns.items() if v['image_id'] in self.ids}
        self.coco.imgToAnns = defaultdict(list, {k: v for k, v in self.coco.imgToAnns.items() if k in self.ids})
