...
class TrainAugmentation(Compose):
    def __init__(self, size, bg_color='black',
        ...
        # deform
        deform = [RandomSkew(random_skew) if random_skew > 0 else None,
                  RandomRotate(random_rotate) if random_rotate > 0 else None,
                  GridDistortion() if grid_distortion else None,
                  ElasticDeformation(approximate=True) if elastic_deformation else None,
                  ]
        deform = [d for d in deform if d]
        deform = RandomChoice(deform)
...
def expand_dataset_by_flips(dataset, hflip=False, vflip=False, dflip=False):
    if hflip:
        expand = dataset >> Compose([HorizontalFlip(), ImageIdAppend('H')])
        dataset = dataset + expand

    if vflip:
        expand = dataset >> Compose([VerticalFlip(), ImageIdAppend('V')])
        dataset = dataset + expand

    if dflip:
        expand = dataset >> Compose([Transpose(), ImageIdAppend('T')])
        dataset = dataset + expand
