...
def efficient_det(scaling, min_sizes=(32, 64, 128, 256, 512)):
    ...
    return {'min_dim': Rinput,
            ...
            'class_output': output,
            'dropout': 0,
            'pretrained': {'coco': {'efficientnet-b%d' % scaling: 'model_zoo:' + coco_pretrained[scaling]}}
            }
...
class SingleShotDetector(nn.Module):
    ...
    def predict(self, image, tta=None, image_size=None, conf_thresh=0.01, nms_thresh=0.15, max_bbox=0, mask_thresh=-1, mask2rbox=False, **kwargs):
        """predict in single pass"""
        with torch.no_grad():
            x, height_width = self.test_transform.pre_process(image, min_dim=image_size, data_aug=tta)
