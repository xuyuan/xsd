...
from .utils.logger import Logger, default_log_dir
...
class Trainer(object):
    ...
    def _init(self, device, model, datasets, criterion):
        ...
        self.logger = None
        if self.is_main_process:
            self.logger = Logger(log_dir, mlflow_tracking_uri=args.mlflow_tracking_uri)
            with (log_dir / 'args.yml').open('w') as f:
                f.write(args_yaml)

            hparam_dict = self.get_hparam()
            self.logger.add_hparams(hparam_dict)
            ...
            if args.write_graph:
                ...
                with torch.no_grad():
                    ...
                    self.logger.add_graph(model, images)
    ...
    def run_epoch(self, epoch, phase):
        ...
        if is_train:
            ...
            if self.logger:
                self.logger.add_scalar('learning_rate', self.get_lr(), epoch)
        ...
        for batch in pbar:
            ...
            if phase == 'train':
                ...
                if self.lr_scheduler and self.lr_scheduler.name == 'findlr':
                    ...
                    if self.logger:
                        self.logger.add_scalar('learning_rate', self.get_lr(), self.iteration)
            elif phase == 'valid':
                it += 1
            ...
            if self.logger and optimize_step and self.args.log_loss_interval > 0 and self.iteration % self.args.log_loss_interval == 0:
                # tb_writer.add_scalars('Loss', batch_loss_dict, iteration)
                for k, v in batch_loss_dict.items():
                    self.logger.add_scalar(phase + '/Loss/' + k, v, epoch)
            ...
            if self.args.log_images:
                ...
                for name, batch in name_batch.items():
                    if batch is not None:
                        ...
                        self.logger.add_image('/'.join([phase, name]), images_grid, epoch)

            # scalars = {phase + k: v for k, v in epoch_loss_dict.items()}
            # tb_writer.add_scalars('EpochLoss', scalars, epoch)
            for k, v in epoch_loss_dict.items():
                self.logger.add_scalar(phase + '/EpochLoss/' + k, v, epoch)
    ...
    def run(self, model, datasets, criterion, report_intermediate_result=None):
        try:
            ...
            if self.is_main_process:
                ...
                hparam_dict = self.get_hparam()
                metric_dict = {'max_metric_score': self.max_metric_score, 'min_epoch_loss': self.min_epoch_loss}
    ...
    def get_hparam(self):
        nonhparam_args = ('comment', 'write_graph', 'log_dir', 'local_rank', 'mlflow_tracking_uri')
        return {k: v for k, v in vars(self.args).items() if k not in nonhparam_args and v is not None}
    ...
    def _run(self, report_intermediate_result=None):
        ...
        pbar_epoch = trange(self.start_epoch + 1, self.args.max_epochs + 1,
                            initial=self.start_epoch, total=self.args.max_epochs,
            ...
            for phase in self.data_loaders:
                ...
                if self.is_main_process:
                    ...
                    if evaluation is not None:
                        ...
                        self.logger.add_scalar('test/metric', metric_score, epoch)
                        for k, v in evaluation.items():
                            if isinstance(v, Number):
                                self.logger.add_scalar('test/' + k.replace(' ', '_'), v, epoch)
                            elif isinstance(v, dict) and 'score' in v:
                                self.logger.add_scalar('test/' + k.replace(' ', '_'), v['score'], epoch)
...
class ArgumentParser(object):
    def __init__(self, description=__doc__):
        ...
        group.add_argument('--log-dir', type=str, default=default_log_dir(),
                           help='Location to save logs and checkpoints')
        group.add_argument('--mlflow-tracking-uri', type=str,
